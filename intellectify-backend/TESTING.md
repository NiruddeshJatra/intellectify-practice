...
```bash
yarn test -- tests/adminAuth.test.js
```
...
- **jsonwebtoken (v9.0.2)**: Token verification
...
### 2. Admin Authentication Tests (`adminAuth.test.js`)
- Admin user management
- Password hashing and validation
- Admin-specific routes protection
- Session management

### 3. Content Service Tests (`contentService.test.js`)
...
### 4. Image Service Tests (`imageService.test.js`)
- File validation
- MIME type handling
- Image processing
- Storage management

### 5. Security Tests (`security.test.js`, `security-integration.test.js`)
...
### 6. Temporary Image Upload Tests (`temp-image-upload.test.js`)
...
- `test:manual`: Run manual auth tests
- `lint`: Run ESLint
...
3. **GET** `/api/content?category=TECH_INSIGHTS`
