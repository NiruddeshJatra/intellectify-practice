...
class ImageService {
  ...
  async initializeDirectories() {
    try {
      ...
      throw new AppError('Failed to initialize upload directories', 500, 'UPLOAD_INIT_FAILED');
    }
  }
 ...
 */
  ...
  async moveImageFromTemp(tempPath, category = 'GENERAL', contentId) {
    try {
      ...
      if (!(await this.imageExists(path.join('..', 'temp', tempPath)))) {
        throw new AppError('Temporary image file not found', 404, 'TEMP_FILE_NOT_FOUND');
      }
  ...
  async deleteFiles(paths, { recursive = false, force = false } = {}) {
    ...
    for (const filePath of pathsArray) {
      try {
        ...
        const fullPath = filePath.startsWith(this.baseUploadPath) || 
  ...
  async cleanupContentImages(contentId, category = 'GENERAL') {
    ...
    const result = await this.deleteFiles(fullContentPath, { recursive: true, force: true });
  ...
  async cleanupTempFiles() {
    ...
    try {
      ...
      const result = await this.deleteFiles(filesToDelete, { recursive: true, force: true });
      
      if (result.deleted.length > 0) {
        console.log(`Cleaned up ${result.deleted.length} temporary files and directories`);
      }
